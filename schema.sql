...
/**
* CHAT_MESSAGES
* Note: This table stores chat messages for each user.
*/
...
/**
* USER_CREDITS
* Note: This table keeps track of user credits for non-subscribers.
*/
...
-- Trigger to initialize user credits on user creation
...
create function public.decrement_user_credits(user_id uuid) 
returns void as $$
begin
...
-- Add a credits column to the users table for easier access
alter table users add column credits integer;

-- Function to sync credits from user_credits to users table
create function public.sync_user_credits() 
returns trigger as $$
begin
  update public.users
...
-- Trigger to sync credits on update
